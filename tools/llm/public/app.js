(() => {
  ...
  async function sendQuestion() {
    ...
    // Prefer LAN automatically when a host is configured; otherwise fall back
    // to the local LLM on the taunt server.
    const selectedLlmSource = lanHost ? "lan" : "local";
    ...
    // Many LAN-hosted Responses servers (e.g., basic SmolLM builds) do not
    // support explicit "reasoning" fields. To avoid errors like
    // "Reasoning setting 'high' cannot be converted", we suppress reasoning
    // when using a LAN source.
    const effectiveReasoning =
      selectedLlmSource === "lan" ? "none" : selectedReasoning;
